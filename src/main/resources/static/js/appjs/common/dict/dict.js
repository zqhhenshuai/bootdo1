...
// $('#exampleTable').bootstrapTable('mergeCells', {index: 1, field: 'name', colspan: 2, rowspan: 3});
function load() {
	selectLoad();
	$('#exampleTable')
		.bootstrapTable(
			{
				...
				onLoadSuccess:function(data){
                    ...
                    for(var i=0;i<data.rows.length;i++) {
                        curorderid=data.rows[i].id;
						if(curorderid!=data.rows[i].id){
                            $('#exampleTable').bootstrapTable('mergeCells', {index: i, field: 'name', rowspan: 2});
